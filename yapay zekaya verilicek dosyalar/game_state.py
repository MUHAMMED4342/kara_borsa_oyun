...
class GameState:
    ...
    def hire_employee(self, name: str, city: str) -> tuple:
        ...
        karaborsa işi çevirir; hiçbir şirket kurmaz, aklama yapmaz. Oyuncu
        sadece kiralama masrafını öder ve 30 günde bir maaş verir."""
    ...
    def process_employees_daily(self) -> list:
        """Her gün: her adam kendi şehrinde karaborsa işi çevirip kirli para
        üretir (şirket YOK, aklama YOK - aklama sadece oyuncunun kendi
        şirketi üzerinden yapılır). 30 günde bir oyuncudan maaşını alır.
        Maaş ödenemezse adam sizi terk eder."""
        messages = []
        to_remove = []
        ...
        for e in self.employees:
            ...
            # Tamamı kirli para: bir şirketten geçmediği sürece temizlenmez.
            self.dirty_cash += gross
            self.cash += gross
            if self.cash > self.highest_cash:
                self.highest_cash = self.cash
