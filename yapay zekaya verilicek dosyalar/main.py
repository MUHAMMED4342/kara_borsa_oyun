...
from dialogs import (
    ...
    LaunderDialog, BankLoanDialog, LandLoanDialog, BankingDialog, JailDialog,
    ...
    def on_next_day(self, event):
        ...
        if self.state.loan_amount > 0:
            ...
            if not success:
                ...
                speak(f"Kredi temerrüdü. Şirket kapatıldı. {msg}")
                self.refresh_product_list()
                self.update_wallet_display()
                self.auto_save()
                return
        ...
        if police["caught"]:
            ...
            if police["can_bribe"] and self.state.cash > 0:
                msg = (f"POLİS YAKALADI!\n\n"
                       f"Kirli para: {format_tl(self.state.dirty_cash)} TL\n"
                       f"Rüşvet miktarı: {format_tl(bribe_amount)} TL\n"
                       f"Rüşvet ver?")
                ...
                if dlg.ShowModal() == wx.ID_YES:
                    if self.state.bribe_police(bribe_amount):
                        speak(f"Rüşvet verdiniz. {format_tl(bribe_amount)} TL ödendi")
                        self.update_wallet_display()
                        self.refresh_product_list()
                        self.auto_save()
                        dlg.Destroy()
                        ...
                        msgs = []
                        if bank_interest > 0:
                            msgs.append(f"Banka faizi: {format_tl(bank_interest)} TL")
                        if events:
                            msgs.extend(events)
                        if msgs:
                            speak(" ".join(msgs))
                        ...
                        # SKOR GÜNCELLEME: Her 3 günde bir
