...
def _merge_entry(score_data: List[Dict], username: str, total: float,
    ...
    for entry in score_data:
        if entry.get("username") == username:
            existing_total = entry.get("total", entry.get("score", 0))
            if total > existing_total:
                ...
                _log(f"[Bilgi] Mevcut toplam ({format_tl(existing_total)}) yeni toplamdan ({format_tl(total)}) yüksek.")
                return score_data, False, (
                    f"Mevcut toplamınız ({format_tl(existing_total)} TL) yeni toplamdan "
                    f"({format_tl(total)} TL) daha yüksek. Güncelleme yapılmadı."
                )
